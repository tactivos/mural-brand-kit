# PDF Reskin Log

Tracks all 39 existing Mural 1-pagers by document type, reskin status, and notes.

**Statuses:** Catalogued → Selected → Reskinned → Audited

---

## Product one-sheet

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| 1 | Mural Overview One-pager.pdf | Selected | — | General Mural pitch |
| 2 | MURAL_Overview_One Sheet_Release.pdf | Selected | — | Updated release version |
| 3 | Hybrid Collaboration_One Sheet_Final.pdf | Selected | — | Hybrid work angle |
| 4 | Customer Success.pdf | Catalogued | — | Could also be use-case |
| 5 | Onboarding Specialist.pdf | Catalogued | — | Role-specific overview |
| 6 | Types of Collaborators.pdf | Catalogued | — | Persona breakdown |

## Use case

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| 1 | Mural for Agile Teams.pdf | Selected | — | Audience-specific use case |
| 2 | Mural for Consultants.pdf | Selected | — | Audience-specific use case |
| 3 | Mural for PDE.pdf | Selected | — | Product/Design/Eng audience |
| 4 | Impact of MURAL infographic.pdf | Catalogued | — | Infographic format, ROI focus |
| 5 | MURAL_Luma System_One Sheet_Final.pdf | Catalogued | — | LUMA system use case |

## Competitive comparison

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| — | (no existing PDFs) | — | — | Starter template only |

## Pricing guide

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| 1 | Mural Member Pricing Overview.pdf (w/ platform fee) | Selected | — | March 2026, with platform fee |
| 2 | Mural Member Pricing.pdf (w/o platform fee) | Selected | — | March 2026, without platform fee |
| 3 | Selecting the Right Mural Plan.pdf | Selected | — | Plan comparison |
| 4 | Member-pricing-model_transition-overview.pdf | Catalogued | — | Transition explainer |
| 5 | Mural Enterprise Plan - Member Pricing Technical Overview.pdf | Catalogued | — | Technical pricing detail |
| 6 | Member pricing Enterprise Plan.pdf | Catalogued | — | Enterprise-specific pricing |
| 7 | Mural QEM Pricing.pdf | Catalogued | — | QEM-specific |
| 8 | Switch to Mural_2023.pdf | Catalogued | — | Older competitive switch |
| 9 | Why members select Enterprise.pdf | Catalogued | — | Could also be product one-sheet |

## Topic deep-dive

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| 1 | Mural Enterprise Data Security Guide (1).pdf | Selected | — | Security deep-dive |
| 2 | Mural Identity and Access Management.pdf | Selected | — | IAM deep-dive |
| 3 | Enterprise Grade One-pager.pdf | Selected | — | Enterprise capabilities overview |
| 4 | Mural Enterprise Workspace Strategy Guide.pdf | Catalogued | — | Workspace strategy |

## Program overview

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| 1 | LUMA-FCP.pdf | Selected | — | Facilitation Certification Program |
| 2 | LUMA-PCP.pdf | Reskinned | luma-practitioner-certification-program.html | Reference example for all reskins |
| 3 | LUMA-Enterprise Buyer.pdf | Selected | — | Enterprise buyer program |
| 4 | LUMA-ICP.pdf | Catalogued | — | Innovation Certification Program |
| 5 | Luma System-One Sheet.pdf | Catalogued | — | LUMA System overview |
| 6 | LUMA-Workplace.pdf | Catalogued | — | Workplace program |

## Partnership one-sheet

| # | Source PDF | Status | Reskin file | Notes |
|---|-----------|--------|-------------|-------|
| 1 | Microsoft Strat Partnership.pdf | Selected | — | Strategic partnership overview |
| 2 | Copilot and Mural AI.pdf | Selected | — | AI integration partnership |
| 3 | ADO One-Pager.pdf | Selected | — | Azure DevOps integration |
| 4 | msft-strategic-partnership.pdf | Catalogued | — | Older version of #1? |
| 5 | Copilot and Graph Connector.pdf | Catalogued | — | Graph connector integration |
| 6 | Getting Started with the Mural App for Microsoft Teams.pdf | Catalogued | — | Teams integration guide |
| 7 | Microsoft Teams integration one pager 2.pdf | Catalogued | — | Teams integration v2 |
| 8 | MACC One Pager.pdf | Catalogued | — | Microsoft commerce |

---

## Summary

| Type | Total | Selected | Reskinned |
|------|-------|----------|-----------|
| Product one-sheet | 6 | 3 | 0 |
| Use case | 5 | 3 | 0 |
| Competitive comparison | 0 | 0 | 0 |
| Pricing guide | 9 | 3 | 0 |
| Topic deep-dive | 4 | 3 | 0 |
| Program overview | 6 | 3 | 1 |
| Partnership one-sheet | 8 | 3 | 0 |
| **Total** | **38** | **18** | **1** |
